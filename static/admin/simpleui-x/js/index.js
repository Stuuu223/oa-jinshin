(function () {
    ...
    function openByHash() {
        ...
        for (var i = 0; i < app.menuData.length; i++) {
            ...
            if ((item.url || '/') == hash) {
                // 修复:hash 直链访问(如 #/admin/consult-workbench/)时,若 tabs 已有该 url 才 selected=true(仅激活),
                // 否则 selected=false 创建新 tab——否则 iframe 无内容空白
                var hasTab = false;
                for (var j = 0; j < app.tabs.length; j++) {
                    if (app.tabs[j].url == item.url) { hasTab = true; break; }
                }
                app.openTab(item, item.eid, hasTab, false);
                break;
            }
        }
    }
